...
Deve-se depender de abstrações e não de implementações concretas.
...
Ideal para manter instância única de conexão com o banco de dados.
...
Facilita criação de objetos `Pet` sem expor lógica complexa.

```javascript
function PetFactory(tipo, dados) {
	if (tipo === "cachorro") return { ...dados, tipo: "Cachorro" };
	if (tipo === "gato") return { ...dados, tipo: "Gato" };
}
const pet = PetFactory("cachorro", { nome: "Rex", idade: 2 });
```
